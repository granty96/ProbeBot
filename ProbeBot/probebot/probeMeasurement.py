...
def __extract_magnification(image) -> float:
...
def __solve_for_y(poly_coeffs, y):
    pc = poly_coeffs.copy()
    pc[-1] -= y
    return np.roots(pc).real


def measureTip(orientation):
    ...
    try:
        ...
        image_blurred = cv2.cvtColor(
            cv2.GaussianBlur(roi, (5, 5), 0), cv2.COLOR_BGR2HSV
        )
        ...
        left_partition = [
            point for point in contour_points if point[0] < criticalPoint[0]
        ]
        right_partition = [
            point for point in contour_points if point[0] > criticalPoint[0]
        ]
        ...
        left_partition_fit = np.polyfit(
            left_partition_x, left_partition_y, left_fit_degree
        )
        ...
        for i in range(width):
            cv2.circle(
                roi, (i, int(left_partition_fit_function(i))), 2, (0, 0, 255), -1
            )

        for i in range(width):
            cv2.circle(
                roi, (i, int(right_partition_fit_function(i))), 2, (0, 0, 255), -1
            )
        ...
        left_roots = __solve_for_y(left_partition_fit, highestPoint[1])
        ...
        right_roots = __solve_for_y(right_partition_fit, highestPoint[1])
        ...
        cv2.line(
            roiCopy, left_measurement_point, right_measurement_point, (0, 255, 255), 2
        )
